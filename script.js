...
function showToast(message, type = 'success') {
  ...
  // Services data
  const services = [
    { id: 1, name: "Men's Haircut", price: 299, image: "img/mens_haircut.jpg", category: "mensSalon" },
    { id: 2, name: "Beard Trim", price: 199, image: "img/beard_trim.jpg", category: "mensSalon" },
    { id: 3, name: "Men's Facial", price: 499, image: "img/mens_facial.jpg", category: "mensSalon" },
    { id: 4, name: "Hair Coloring", price: 799, image: "img/hair_coloring.jpg", category: "mensSalon" },
    { id: 5, name: "Head Massage", price: 249, image: "img/head_massage.jpg", category: "mensSalon" },
    { id: 6, name: "Women's Haircut", price: 499, image: "img/womens_haircut.jpg", category: "girlsSalon" },
    { id: 7, name: "Manicure", price: 399, image: "img/manicure.jpg", category: "girlsSalon" },
    ...
    { id: 16, name: "Salon at Home", price: 999, image: "img/mens_hair_cut.png", category: "mostBooked" },
  ...
  function initSearch() {
    ...
    searchInput.addEventListener('input', function() {
      ...
      if (searchTerm.length > 0) {
        const matchingServices = allServices.filter(service => service.toLowerCase().includes(searchTerm));
        if (matchingServices.length > 0) {
          searchBox.innerHTML = matchingServices.map(service =>
            `<div class="search-suggestion" onclick="selectSearchSuggestion('${service}')">${service}</div>`
          ).join('');
    ...
    searchInput.addEventListener('keydown', function(e) {
      if (e.key === 'Enter') {
        ...
        if (firstSuggestion) {
          selectSearchSuggestion(firstSuggestion.textContent);
        }
      }
    });
  }
  ...
  function highlightService(service) {
    ...
    serviceCards.forEach(card => {
      ...
      if (cardTitle && cardTitle.textContent === service.name) {
        card.scrollIntoView({ behavior: 'smooth', block: 'center' });
        card.classList.add('highlight-service');
        setTimeout(() => card.classList.remove('highlight-service'), 2000);
        found = true;
      }
    });
    if (!found) {
      showToast(`Service "${service.name}" found. Scroll to view all services.`, 'info');
    }
  }
